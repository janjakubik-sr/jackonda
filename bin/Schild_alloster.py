...
if count == 0:
    ...
    if os.path.isfile("Schild_alloster.res"):
        os.remove("Schild_alloster.res")
    res = open("Schild_alloster.res","w")
    res.write("Meta-analysis: KBs from dose response shifts\nAllosteric inhibition\n\n")
    res.write("# pKB\talpha\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("Schild_alloster.draw"):
        os.remove("Schild_alloster.draw")
    draw = open("Schild_alloster.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 3 or (x_max - x_min) < 1.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                def func(x, KB, alpha):
                    import numpy as np
                    return np.log10((10**x)*(1-alpha)/((10**x)*alpha + (10**KB)))
                ...
                res.write(str(KB_res)+"\t"+str(alpha_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"K\\sB\\N="+str("{0:.2f}".format(KB_calc))+' \\#{B1} '+str("{0:.2f}".format(KB_err))+"; \\f{Symbol}a\\f{}="+str("{0:.4f}".format(alpha_calc))+' \\#{B1} '+str("{0:.4f}".format(alpha_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_Schild_alloster_fit.data')
                ...
                #save curve
                curve=(str(base)+'_Schild_alloster_curve.data')
