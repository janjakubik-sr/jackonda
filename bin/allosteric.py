...
if count == 0:
    ...
    if os.path.isfile("allosteric.res"):
        os.remove("allosteric.res")
    res = open("allosteric.res","w")
    res.write("Allosteric interaction\n\n")
    res.write("logKB\t\t\talpha\t\tKA\t\t[X]\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("alloster.draw"):
        os.remove("alloster.draw")
    draw = open("alloster.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 3:
            ...
            if x_min < -15 or x_max > 0:
                ...
                res.write(str(KB_res)+"\t"+str(alpha_res)+"\t"+str(KA)+"\t"+str(CA)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                if alpha_calc > 1:
                    draw.write("s"+str(c)+" legend \"logK\\sA\\N="+str("{0:.2f}".format(KB_calc))+' \\#{B1} '+str("{0:.2f}".format(KB_err))+"; \\f{Symbol}a\\f{}="+str("{0:.2f}".format(alpha_calc))+' \\#{B1} '+str("{0:.2f}".format(alpha_err))+"; "+str(base)+"\" \n")
                else:
                    draw.write("s"+str(c)+" legend \"logK\\sA\\N="+str("{0:.2f}".format(KB_calc))+' \\#{B1} '+str("{0:.2f}".format(KB_err))+"; \\f{Symbol}a\\f{}="+str("{0:.4f}".format(alpha_calc))+' \\#{B1} '+str("{0:.4f}".format(alpha_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_alloster_fit.data')
                ...
                #save curve
                curve=(str(base)+'_alloster_curve.data')
