...
if count == 0:
    ...
    if os.path.isfile("decay_1.res"):
        os.remove("decay_1.res")
    res = open("decay_1.res","w")
    res.write("decay from one site\n\n")
    res.write("Koff\t\t\tns\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("decay_1.draw"):
        os.remove("decay_1.draw")
    draw = open("decay_1.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 4:
            ...
            if x_min < 0 or y_max < 0 or y_max > 1.2:
                ...
                ns_max = 1
                #fit data
                p0 = np.array([Koff_estim, ns_estim])
                def func(x, Koff, ns):
                    return ns + (1-ns) * np.exp(-Koff * x)
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([Koff_min, ns_min],[Koff_max, ns_max]))
                ...
                res.write(str(Koff_res)+"\t"+str(ns_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"K\\sOff\\N="+str("{0:.4f}".format(Koff_calc))+' \\#{B1} '+str("{0:.4f}".format(Koff_err))+"ns="+str("{0:.4f}".format(ns_calc))+' \\#{B1} '+str("{0:.4f}".format(ns_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_decay_1_fit.data')
                ...
                #save curve
                curve=(str(base)+'_decay_1_curve.data')
