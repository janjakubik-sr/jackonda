...
if count == 0:
    ...
    if os.path.isfile("competition_0.res"):
        os.remove("competition_0.res")
    res = open("competition_0.res","w")
    res.write("Competition with low affinity ligand\nHill coeff = 1\n\n")
    res.write("logIC50\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("comp_0.draw"):
        os.remove("comp_0.draw")
    draw = open("comp_0.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 2.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                #save curve
                curve=(str(base)+'_comp_0_curve.data')
