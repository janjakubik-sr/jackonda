...
if count == 0:
    ...
    if os.path.isfile("FR_OM_2.res"):
        os.remove("FR_OM_2.res")
    res = open("FR_OM_2.res","w")
    ...
    if os.path.isfile("FR_OM_2.draw"):
        os.remove("FR_OM_2.draw")
    draw = open("FR_OM_2.draw","w")
    c=-1 #plot counter
    d=0  #symbolcounter
    #get system Emax
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 2.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                tau_max = 1000
                #fit data
                p0 = np.array([tau_estim, pKA_estim,])
                def func(x, tau, pKA):
                    return basal + ((10**x) * tau * (Emax - basal)) / ((10**x) * (1 +tau) + (10**pKA))
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([tau_min,pKA_min],[tau_max,pKA_max]))
                ...
                res.write(str(tau_res)+"\t"+str(pKA_res)+"\t"+str(Emax)+"\t"+str(basal)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau_calc))+' \\#{B1} '+str("{0:.4f}".format(tau_err))+"; logK\\sA\\N ="+str("{0:.2f}".format(pKA_calc))+' \\#{B1} '+str("{0:.2f}".format(pKA_err))+"; E\\sMAX\\N = "+str("{0:.2f}".format(Emax))+"; basal = "+str("{0:.2f}".format(basal))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_FR_OM_2_fit.data')
                ...
                #save curve
                curve=(str(base)+'_FR_OM_2_curve.data')
