...
if columns == 0:
    ...
    if np.size(x_data_1) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            Emax1_min = 1 + (Emax1_estim - basal) * 0.1
            Emax1_max = 1 + (Emax1_estim - basal) * 10
            tau1_estim = 1
            tau1_min = 0.001
            tau1_max = 1000
...
if columns == 0:
    ...
    if np.size(x_data_2) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau2_calc))+' \\#{B1} '+str("{0:.4f}".format(tau2_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax2_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax2_err))+"; K\\sA\\N="+str("{0:.2f}".format(pKA2))+"; "+str(file_2)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
if columns == 0:
    ...
    if np.size(x_data_2) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau3_calc))+' \\#{B1} '+str("{0:.4f}".format(tau3_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax3_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax3_err))+"; K\\sA\\N="+str("{0:.2f}".format(pKA3))+"; "+str(file_3)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
if np.any(x_data_1 != x_data_2) or np.any(x_data_1 != x_data_3):
    ...
    pg_estim = [p1_opt[0], p1_opt[1], p2_opt[0], p3_opt[0]]
