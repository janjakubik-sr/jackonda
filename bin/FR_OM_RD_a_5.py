...
if np.any(x_data_1 != x_data_2) or np.any(x_data_1 != x_data_3) or np.any(x_data_1 != x_data_4) or np.any(x_data_1 != x_data_5):
    ...
    res.write("\nGlobal fit with shared Emax and pKA\n")
    #fit data
    def func2(x, p):
        tau, Emax, pKA = p
        return basal + ((10**x) * tau * (Emax - basal)) / ((10**x) * (1 +tau) + (10**pKA))
    ...
    pg_estim = [p1_opt[0], p1_opt[1], p1_opt[2], p2_opt[0], p3_opt[0], p4_opt[0], p5_opt[0]]
    ...
    pg2_opt = [pg_opt[3], pg_opt[1],pg_opt[2]]
    pg3_opt = [pg_opt[4], pg_opt[1],pg_opt[2]]
    pg4_opt = [pg_opt[5], pg_opt[1],pg_opt[2]]
    pg5_opt = [pg_opt[6], pg_opt[1],pg_opt[2]]
    tau1_calc = (pg_opt[0])
    tau1_err = (pg_err[0])
    Emax1_calc = (pg_opt[1])
    Emax1_err = (pg_err[1])
