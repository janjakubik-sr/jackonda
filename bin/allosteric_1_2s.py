...
if count == 0:
    ...
    if os.path.isfile("allosteric_1_2s.res"):
        os.remove("allosteric_1_2s.res")
    res = open("allosteric_1_2s.res","w")
    res.write("Allosteric interaction - One allosteric modulator at 2 sites\n")
    res.write("gamma=1, delta=1\n\n")
    res.write("logKB1\t\talpha1\t\tlogKB2\talpha2\t\tKA\t[X]\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("alloster_1_2s.draw"):
        os.remove("alloster_1_2s.draw")
    draw = open("alloster_1_2s.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 9 or (x_max - x_min) < 4:
            ...
            if x_min < -15 or x_max > 0:
                ...
                dlg.Destroy()            
                #fit data
                p0 = np.array([KB1_estim, alpha1_estim, KB2_estim, alpha2_estim])
                def func(x, KB1, alpha1, KB2, alpha2):
                    global CA
                    CA = float(CA)
                    global KA
                    KA = float(KA)
                    return 100 * (CA + KA) / (CA + KA * (1 + (10**x) / (10**KB1) + (10**x)/(10**KB2) * (1 + (10**x)/(10**KB1)) ) / (1 + (10**x)/alpha1/(10**KB1) + (10**x)/alpha2/(10**KB2) * (1 + (10**x)/alpha1/(10**KB1) ) ) )
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([KB1_min,alpha1_min,KB2_min,alpha2_min],[KB1_max,alpha1_max,KB2_max,alpha2_max]))
                ...
                res.write(str(KB1_res)+"\t"+str(alpha1_res)+"\t"+str(KB2_res)+"\t"+str(alpha2_res)+"\t"+str(KA)+"\t"+str(CA)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                #save curve
                curve=(str(base)+'_alloster_1_2s_curve.data')
