...
#results log
if os.path.isfile("FR_OMAMA_3D.res"):
    os.remove("FR_OMAMA_3D.res")
res = open("FR_OMAMA_3D.res","w")

#prepare file for Grace plot
if os.path.isfile("FR_OMAMA_3D.draw"):
    os.remove("FR_OMAMA_3D.draw")
draw = open("FR_OMAMA_3D.draw","w")
c=-1 #plot counter
d=0 #symbolcounter


global Emax, basal, KA, KB, tauA, tauB, alpha, beta, B1, B2, B3, B4
...
class Estimates(wx.Dialog):
    def __init__(
        ...
        label = wx.StaticText(self, -1, "OMAMA - parameter set-up: 4 curves including control.")
...
#select files
dialog = wx.FileDialog(None, "Choose a control curve, [B] = "+str(B1)+"\n", os.getcwd(), "","*.dat", wx.FD_OPEN)
...
dialog = wx.FileDialog(None, "Choose a curve with the lowest concentration, [B] = "+str(B2)+"\n", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
dialog = wx.FileDialog(None, "Choose a curve with the intermediate concentration, [B] = "+str(B3)+"\n", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
dialog = wx.FileDialog(None, "Choose a control the highest concentration, [B] = "+str(B4)+"\n", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
#fit data
p1 = [alpha_ini, beta_ini]
#Define global fit function
def func_glob(x, y, p):
    alpha, beta = p
    return basal + Emax * (tauA * (10**x) * (KB + alpha * beta * y) + tauB * y * KA) / ((10**x) * KB + KA * KB + KA * y + alpha * (10**x) * y + tauA * (10**x) * (KB + alpha * beta * y) + tauB * y * KA)
def err(p, x, y, z):
...
np.savetxt('Data_A_OMAMA_3D.fit.data',xz_calc_1,fmt='%.4e')
#get color
i = i + 1
color = (color_list[i])
# plot set 1
...
draw.write("s"+str(c)+" legend \" \\f{Symbol}a\\f{}="+str("{0:.2f}".format(alpha_calc))+' \\#{B1} '+str("{0:.2f}".format(alpha_err))+" \\f{Symbol}b\\f{}="+str("{0:.2f}".format(beta_calc))+' \\#{B1} '+str("{0:.2f}".format(beta_err))+"\" \n")
c=(c+1)
e=(c-1)
fit=('Data_A_OMAMA_3D.fit.data')
...
np.savetxt('Data_B_OMAMA_3D.fit.data',xz_calc_2,fmt='%.4e')
#get color
i = i + 1
color = (color_list[i])
# plot set 2
...
np.savetxt('Data_C_OMAMA_3D.fit.data',xz_calc_3,fmt='%.4e')
#get color
i = i + 1
color = (color_list[i])
# plot set 3
...
np.savetxt('Data_D_OMAMA_3D.fit.data',xz_calc_4,fmt='%.4e')
#get color
i = i + 1
color = (color_list[i])
# plot set 4
