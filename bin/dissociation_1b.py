...
if count == 0:
    ...
    if os.path.isfile("dissociation_1b.res"):
        os.remove("dissociation_1b.res")
    res = open("dissociation_1b.res","w")
    res.write("Dissociation from one site\n\n")
    res.write("Koff\t\t\t\t\tBeq\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("dissoc_1b.draw"):
        os.remove("dissoc_1b.draw")
    draw = open("dissoc_1b.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 4:
            ...
            if x_min < 0 or y_max < 0:
                ...
                Beq_max = 1.1 * y_max
                #fit data
                p0 = np.array([Koff_estim, Beq_estim])
                def func(x, Koff, Beq):
                    import numpy
                    return Beq * numpy.exp(-Koff * x)
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([Koff_min, Beq_min],[Koff_max, Beq_max]))
                ...
                Beq_res = (str("{0:.0f}".format(Beq_calc))+' ± '+str("{0:.0f}".format(Beq_err)))
                res.write(str(Koff_res)+"\t\t"+str(Beq_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"K\\sOff\\N="+str("{0:.4f}".format(Koff_calc))+' \\#{B1} '+str("{0:.4f}".format(Koff_err))+"; B\\seq\\N="+str("{0:.0f}".format(Beq_calc))+' \\#{B1} '+str("{0:.0f}".format(Beq_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_dissoc_1b_fit.data')
                ...
                #save curve
                curve=(str(base)+'_dissoc_1b_curve.data')
