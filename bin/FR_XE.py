...
if count == 0:
    ...
    if os.path.isfile("FR_XE.res"):
        os.remove("FR_XE.res")
    res = open("FR_XE.res","w")
    res.write("Functional response - one effector\n\n")
    res.write("# logEC50\tEmax\tnH\tbasal\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("FR_XE.draw"):
        os.remove("FR_XE.draw")
    draw = open("FR_XE.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 2.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                #set Emax value
                global Emax
                ask = ('Enter Emax value for ' +str(file)+'\n')
                dlg = wx.TextEntryDialog(None, ask, "Input",'3.0')
                if dlg.ShowModal() == wx.ID_OK:
                    Emax = dlg.GetValue()
                dlg.Destroy()
                Emax = float(Emax)
                ...
                nH_max = 4
                #fit data
                p0 = np.array([EC50_estim, nH_estim])
                def func(x, EC50, nH):
                    return basal + (Emax - basal) / (1 + 10**((EC50-x)*nH))
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([EC50_min, nH_min],[EC50_max, nH_max]))
                ...
                res.write(str(EC50_res)+"\t"+str(Emax)+"\t"+str(nH_res)+"\t"+str(basal)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"log EC\\s50\\N="+str("{0:.2f}".format(EC50_calc))+' \\#{B1} '+str("{0:.2f}".format(EC50_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax))+"; n\\H\\N="+str("{0:.2f}".format(nH_calc))+' \\#{B1} '+str("{0:.2f}".format(nH_err))+"; basal = "+str(basal)+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_FR_XE_fit.data')
                ...
                #save curve
                curve=(str(base)+'_FR_XE_curve.data')
