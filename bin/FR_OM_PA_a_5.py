...
if columns == 0:
    ...
    if np.size(x_data_1) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            #calculate estimates
            pKA1_estim = x_min + (x_max - x_min) * 0.5
            pKA1_min = x_min
            pKA1_max = x_max - 0.5
            ...
            tau1_min = 0.001
            tau1_max = 1000
            p1 = [tau1_estim, Emax1_estim, pKA1_estim]
            #fit data
            p1_opt, p1_cov = opt.curve_fit(func, x_data_1, y_data_1, p1, bounds=([tau1_min, Emax1_min, pKA1_min],[tau1_max,Emax1_max, pKA1_max]))
...
#First partial agonist best fit
#select file
...
if columns == 0:
    ...
    if np.size(x_data_2) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            step = (x_max - x_min) * 0.01
            #calculate estimates
            pKA2_estim = pKA1_calc
            pKA2_min = pKA1_calc - 2
            pKA2_max = pKA1_calc + 2
            ...
            p2 = [tau2_estim, Emax2_estim, pKA2_estim]
            #fit data
            p2_opt, p2_cov = opt.curve_fit(func, x_data_2, y_data_2, p2, bounds=([tau2_min, Emax2_min, pKA2_min],[tau2_max,Emax2_max, pKA2_max]))
...
if columns == 0:
    ...
    if np.size(x_data_3) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            step = (x_max - x_min) * 0.01
            #calculate estimates
            pKA3_estim = pKA1_calc
            pKA3_min = pKA1_calc - 2
            pKA3_max = pKA1_calc + 2
            ...
            p3 = [tau3_estim, Emax3_estim, pKA3_estim]
            #fit data
            p3_opt, p3_cov = opt.curve_fit(func, x_data_3, y_data_3, p3, bounds=([tau3_min, Emax3_min, pKA3_min],[tau3_max, Emax3_max, pKA3_max]))
            ...
            res.write(str(tau3_res)+"\t"+str(Emax3_res)+"\t"+str(pKA3_res)+"\t"+str(file_3)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau3_calc))+' \\#{B1} '+str("{0:.4f}".format(tau3_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax3_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax3_err))+"; pK\\sA\\N="+str("{0:.2f}".format(pKA3_calc))+' \\#{B1} '+str("{0:.2f}".format(pKA3_err))+"; "+str(file_3)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
if columns == 0:
    ...
    if np.size(x_data_4) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            step = (x_max - x_min) * 0.01
            #calculate estimates
            pKA4_estim = pKA1_calc
            pKA4_min = pKA1_calc - 2
            pKA4_max = pKA1_calc + 2
            ...
            p4 = [tau4_estim, Emax4_estim, pKA4_estim]
            #fit data
            p4_opt, p4_cov = opt.curve_fit(func, x_data_4, y_data_4, p4, bounds=([tau4_min, Emax4_min, pKA4_min],[tau4_max, Emax4_max, pKA4_max]))
            ...
            res.write(str(tau4_res)+"\t"+str(Emax4_res)+"\t"+str(pKA4_res)+"\t"+str(file_4)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau4_calc))+' \\#{B1} '+str("{0:.4f}".format(tau4_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax4_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax4_err))+"; pK\\sA\\N="+str("{0:.2f}".format(pKA4_calc))+' \\#{B1} '+str("{0:.2f}".format(pKA4_err))+"; "+str(file_4)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
#Fourth partial agonist best fit
#select file
dialog = wx.FileDialog(None, "Choose dat file of second Partial agonist", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
if columns == 0:
    ...
    if np.size(x_data_5) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            step = (x_max - x_min) * 0.01
            #calculate estimates
            pKA5_estim = pKA1_calc
            pKA5_min = pKA1_calc - 2
            pKA5_max = pKA1_calc + 2
            ...
            p5 = [tau5_estim, Emax5_estim, pKA5_estim]
            #fit data
            p5_opt, p5_cov = opt.curve_fit(func, x_data_5, y_data_5, p5, bounds=([tau5_min, Emax5_min, pKA5_min],[tau5_max, Emax5_max, pKA5_max]))
            ...
            res.write(str(tau5_res)+"\t"+str(Emax5_res)+"\t"+str(pKA5_res)+"\t"+str(file_5)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau5_calc))+' \\#{B1} '+str("{0:.4f}".format(tau5_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax5_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax5_err))+"; pK\\sA\\N="+str("{0:.2f}".format(pKA5_calc))+' \\#{B1} '+str("{0:.2f}".format(pKA5_err))+"; "+str(file_5)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
            ...
            #save fit curve
            c=(c+1)
            fit=(str(base_5)+'_FR_OM_PA_fit.data')
            draw.write("read xy \""+str(fit)+"\" \n")
            draw.write("s"+str(c)+" symbol 0 \n")
            draw.write("s"+str(c)+" line type 1 \n")
            draw.write("s"+str(c)+" line color 1 \n")
...
if np.any(x_data_1 != x_data_2) or np.any(x_data_1 != x_data_3) or np.any(x_data_1 != x_data_4) or np.any(x_data_1 != x_data_5):
    ...
    pg_estim = [p1_opt[0], p1_opt[1], p1_opt[2], p2_opt[0], p2_opt[2], p3_opt[0], p3_opt[2], p4_opt[0], p4_opt[2], p5_opt[0], p5_opt[2]]
    ...
    pg3_opt = [pg_opt[5], pg_opt[1],pg_opt[6]]
    pg4_opt = [pg_opt[7], pg_opt[1],pg_opt[8]]
    pg5_opt = [pg_opt[9], pg_opt[1],pg_opt[10]]
    tau1_calc = (pg_opt[0])
    tau1_err = (pg_err[0])
    Emax1_calc = (pg_opt[1])
    Emax1_err = (pg_err[1])
    ...
    c=(c+1)
    fit=(str(base_5)+'_FR_OM_PA_gfit.data')
    draw.write("read xy \""+str(fit)+"\" \n")
    draw.write("s"+str(c)+" symbol 0 \n")
    draw.write("s"+str(c)+" line type 1 \n")
    draw.write("s"+str(c)+" line linestyle 2 \n")
