...
#select file
dialog = wx.FileDialog(None, "Choose first partial agonist dat file", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
if columns == 0:
    ...
    #initial best fit for second partial
...
if np.any(x_data_1 != x_data_2) or np.any(x_data_1 != x_data_3):
    ...
    pg_estim = [p1_opt[0], p1_opt[1], p1_opt[2], p2_opt[0], p2_opt[2], p3_opt[0], p3_opt[2]]
    pg_opt, cov_x, infodict, errmsg, success = opt.leastsq(err_global, pg_estim, args=(x_data_1, y_data_1, y_data_2, y_data_3), full_output=1, epsfcn=0.0001)
    residuals = err_global(pg_opt, x_data_1, y_data_1, y_data_2, y_data_3)
    ...
    #calculate partial agonist curve
    x_calc = np.arange(x_min, x_max, step)
    ...
    # add to plot
    plt.plot(x_calc, y_calc, 'y', linestyle='--')
    #calculate second partial agonist curve
    x_calc = np.arange(x_min, x_max, step)
