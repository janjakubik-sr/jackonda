...
if columns == 0:
    ...
    if np.size(x_data_2) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau2_calc))+' \\#{B1} '+str("{0:.4f}".format(tau2_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax2_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax2_err))+"; K\\sA\\N="+str("{0:.2f}".format(pKA))+"; "+str(file_2)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
if columns == 0:
    ...
    if np.size(x_data_2) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau3_calc))+' \\#{B1} '+str("{0:.4f}".format(tau3_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax3_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax3_err))+"; K\\sA\\N="+str("{0:.2f}".format(pKA))+"; "+str(file_3)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
