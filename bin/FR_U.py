...
if count == 0:
    ...
    if os.path.isfile("FR_U.res"):
        os.remove("FR_U.res")
    res = open("FR_U.res","w")
    res.write("Functional response - double S-shape\n\n")
    res.write("logEC501\tEmax1\t\tlogEC502\tEmax2\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("FR_U.draw"):
        os.remove("FR_U.draw")
    draw = open("FR_U.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 8 or (x_max - x_min) < 4:
            ...
            if x_min < -15 or x_max > 0:
                ...
                res.write(str(EC501_res)+"\t"+str(Emax1_res)+"\t"+str(EC502_res)+"\t"+str(Emax2_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                #save curve
                curve=(str(base)+'_FR_U_curve.data')
