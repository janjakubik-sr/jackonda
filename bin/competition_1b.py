...
if count == 0:
    ...
    if os.path.isfile("competition_1b.res"):
        os.remove("competition_1b.res")
    res = open("competition_1b.res","w")
    res.write("Single site competition\n")
    res.write("logIC50\t\Beq\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("comp_1b.draw"):
        os.remove("comp_1b.draw")
    draw = open("comp_1b.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 6 or (x_max - x_min) < 3:
            ...
            if x_min < -15 or x_max > 0:
                ...
                IC50_max = x_max
                #fit data
                p0 = np.array([IC50_estim, Beq_estim])
                def func(x, IC50, Beq):
                    global Beq_estim, Beq_min, Beq_max
                    Beq_estim = float(Beq_estim)
                    Beq_min = float(Beq_min)
                    Beq_max = float(Beq_max)
                    return Beq - Beq*(10**x) / ((10**IC50) + (10**x))
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([IC50_min,Beq_min],[IC50_max,Beq_max]))
                ...
                res.write(str(IC50_res)+"\t"+str(Beq_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"log IC\\s50\\N="+str("{0:.2f}".format(IC50_calc))+' \\#{B1} '+str("{0:.2f}".format(IC50_err))+"; n\\H\\N="+str("{0:.2f}".format(Beq_calc))+' \\#{B1} '+str("{0:.2f}".format(Beq_err))+"; "+str(base)+"\" \n")
                #calculate fit
                ...
                #save curve
                curve=(str(base)+'_comp_1_curve.data')
