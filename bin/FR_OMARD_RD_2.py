...
if np.any(x_data_1 != x_data_2):
    ...
    def err2_global(p, x, y1, y2):
        p_21 = p[0], p[1], p[2], p[3], p[4]
        p_22 = p[5], p[1], p[2], p[3], p[4]
        err_21 = err_2(p_21, x, y1)
        err_22 = err_2(p_22, x, y2)
        return np.concatenate((err_21, err_22))
    pg_estim = [p1_opt[0], Emax_estim, logKA_estim, alpha_estim, beta_estim, p2_opt[0]]
    pg_opt, cov_x, infodict, errmsg, success = opt.leastsq(err2_global, pg_estim, args=(x_data_1, y_data_1, y_data_2), full_output=1, epsfcn=0.0001)
    residuals = err2_global(pg_opt, x_data_1, y_data_1, y_data_2)
