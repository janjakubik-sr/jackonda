...
if count == 0:
    ...
    if os.path.isfile("association_2.res"):
        os.remove("association_2.res")
    res = open("association_2.res","w")
    res.write("Association with two sites\n\n")
    res.write("Kobs1\t\t\tBeq1\t\tKobs2\t\t\tBeq2\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("assoc_2.draw"):
        os.remove("assoc_2.draw")
    draw = open("assoc_2.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 8:
            ...
            if x_min < 0 or y_max < 0:
                ...
                Beq2_max = y_max
                #fit data
                p0 = np.array([Kobs1_estim, Beq1_estim, Kobs2_estim, Beq2_estim])
                def func(x, Kobs1, Beq1, Kobs2, Beq2):
                    import numpy
                    return Beq1 * (1 - numpy.exp(-Kobs1 * x)) + Beq2 * (1 - numpy.exp(-Kobs2 * x))
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([Kobs1_min, Beq1_min, Kobs2_min, Beq2_min],[Kobs1_max, Beq1_max, Kobs2_max, Beq2_max]))
                ...
                res.write(str(Kobs1_res)+"\t"+str(Beq1_res)+"\t"+str(Kobs2_res)+"\t"+str(Beq2_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                #save curve
                curve=(str(base)+'_assoc_2_curve.data')
