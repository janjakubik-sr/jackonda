...
if count == 0:
    ...
    if os.path.isfile("association_1.res"):
        os.remove("association_1.res")
    res = open("association_1.res","w")
    res.write("Association with one site\n\n")
    res.write("Kobs\t\t\tBeq\t\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("assoc_1.draw"):
        os.remove("assoc_1.draw")
    draw = open("assoc_1.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5:
            ...
            if x_min < 0 or y_max < 0:
                ...
                Kobs_estim = 0.693 / (x_max * 0.3)
                Kobs_min = 0.693 / (x_max * 2)
                Kobs_max = 0.693 / (x_max * 0.001)
                Beq_estim = y_max
                Beq_min = y_max * 0.5
                Beq_max = y_max * 2
                #fit data
                p0 = np.array([Kobs_estim, Beq_estim])
                def func(x, Kobs, Beq):
                    import numpy
                    return Beq * (1 - numpy.exp(-Kobs * x))
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([Kobs_min, Beq_min],[Kobs_max, Beq_max]))
                ...
                Beq_err = (perr[1])
                #write results
                Kobs_res = (str("{0:.4f}".format(Kobs_calc))+' ± '+str("{0:.4f}".format(Kobs_err)))
                Beq_res = (str("{0:.2f}".format(Beq_calc))+' ± '+str("{0:.2f}".format(Beq_err)))
                res.write(str(Kobs_res)+"\t"+str(Beq_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"K\\sObs\\N="+str("{0:.4f}".format(Kobs_calc))+' \\#{B1} '+str("{0:.4f}".format(Kobs_err))+"; B\\sEq\\N="+str("{0:.2f}".format(Beq_calc))+' \\#{B1} '+str("{0:.2f}".format(Beq_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_assoc_1_fit.data')
                ...
                #save curve
                curve=(str(base)+'_assoc_1_curve.data')
