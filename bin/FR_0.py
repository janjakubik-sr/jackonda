    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 2.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                res.write(str(EC50_res)+"\t"+str(Emax_res)+"\t"+str(nH_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"log EC\\s50\\N="+str("{0:.2f}".format(EC50_calc))+' \\#{B1} '+str("{0:.2f}".format(EC50_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax_err))+"; n\\sH\\N="+str("{0:.2f}".format(nH_calc))+' \\#{B1} '+str("{0:.2f}".format(nH_err))+"; "+str(base)+"\" \n")
                #calculate fit
