...
if count == 0:
    ...
    if os.path.isfile("allosteric_1_2.res"):
        os.remove("allosteric_1_2.res")
    res = open("allosteric_1_2.res","w")
    res.write("Allosteric interaction\nOne allosteri modulator at 2 sites\n\n")
    res.write("logKB1\t\talpha1\t\tlogKB2\talpha2\t\tgamma\t\tdelta\t\tKA\t[X]\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("alloster_1_2.draw"):
        os.remove("alloster_1_2.draw")
    draw = open("alloster_1_2.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 5 or (x_max - x_min) < 4:
            ...
            if x_min < -15 or x_max > 0:
                ...
                dlg.Destroy()            
                #fit data
                p0 = np.array([KB1_estim, alpha1_estim, KB2_estim, alpha2_estim, 1, 1])
                def func(x, KB1, alpha1, KB2, alpha2, gamma, delta):
                    global CA
                    CA = float(CA)
                    global KA
                    KA = float(KA)
                    return 100 * (CA + KA) / (CA + KA * (1 + (10**x) / (10**KB1) + (10**x)/(10**KB2) * (1 + (10**x)/gamma/(10**KB1)) ) / (1 + (10**x)/alpha1/(10**KB1) + (10**x)/alpha2/(10**KB2) * (1 + (10**x)/alpha1/gamma/delta/(10**KB1) ) ) )
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([KB1_min,alpha1_min,KB2_min,alpha2_min,0.01,0.01],[KB1_max,alpha1_max,KB2_max,alpha2_max,100,100]))
                ...
                delta_res = (str("{0:.3f}".format(delta_calc))+' ± '+str("{0:.3f}".format(delta_err)))
                res.write(str(KB1_res)+"\t"+str(alpha1_res)+"\t"+str(KB2_res)+"\t"+str(alpha2_res)+"\t"+str(gamma_res)+"\t"+str(delta_res)+"\t"+str(KA)+"\t"+str(CA)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"logK\\sA1\\N="+str("{0:.2f}".format(KB1_calc))+' \\#{B1} '+str("{0:.2f}".format(KB1_err))+"; \\f{Symbol}a\\f{}1="+str("{0:.2f}".format(alpha1_calc))+' \\#{B1} '+str("{0:.2f}".format(alpha1_err))+"\\nlogK\\sA2\\N="+str("{0:.2f}".format(KB2_calc))+' \\#{B1} '+str("{0:.2f}".format(KB2_err))+"; \\f{Symbol}a\\f{}2="+str("{0:.3f}".format(alpha2_calc))+' \\#{B1} '+str("{0:.3f}".format(alpha2_err))+"; \\f{Symbol}g\\f{}="+str("{0:.3f}".format(gamma_calc))+' \\#{B1} '+str("{0:.3f}".format(gamma_err))+"; \\f{Symbol}d\\f{}="+str("{0:.3f}".format(delta_calc))+' \\#{B1} '+str("{0:.3f}".format(delta_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_alloster_1_2_fit.data')
                ...
                #save curve
                curve=(str(base)+'_alloster_1_2_curve.data')
