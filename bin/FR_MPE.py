...
if count == 0:
    ...
    if os.path.isfile("FR_MPE.res"):
        os.remove("FR_MPE.res")
    res = open("FR_MPE.res","w")
    res.write("Functional response - MPE\n\n")
    res.write("# logEC50\tEmax\tnH\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("FR_MPE.draw"):
        os.remove("FR_MPE.draw")
    draw = open("FR_MPE.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 4 or (x_max - x_min) < 1.5:
            ...
            if x_min < -15 or x_max > 0:
                ...
                res.write(str(EC50_res)+"\t"+str(nH_res)+"\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"log EC\\s50\\N="+str("{0:.2f}".format(EC50_calc))+' \\#{B1} '+str("{0:.2f}".format(EC50_err))+"; n\\sH\\N="+str("{0:.2f}".format(nH_calc))+' \\#{B1} '+str("{0:.2f}".format(nH_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_FR_MPE_fit.data')
                ...
                #save curve
                curve=(str(base)+'_FR_MPE_curve.data')
