...
if count == 0:
    ...
    if os.path.isfile("FR_B.res"):
        os.remove("FR_B.res")
    res = open("FR_B.res","w")
    res.write("Functional response - bell-shape\n\n")
    res.write("logEC501\tEmax1\t\tlogEC502\tEmax2\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("FR_B.draw"):
        os.remove("FR_B.draw")
    draw = open("FR_B.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 9 or (x_max - x_min) < 4:
            ...
            if x_min < -15 or x_max > 0:
                ...
                Emax1_min = basal + (y_max - basal) * 0.5
                ...
                #save curve
                curve=(str(base)+'_FR_B_curve.data')
