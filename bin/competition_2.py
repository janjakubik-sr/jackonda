...
if count == 0:
    ...
    if os.path.isfile("competition_2.res"):
        os.remove("competition_2.res")
    res = open("competition_2.res","w")
    res.write("Competition for two sites\n\n")
    res.write("logIC501\tlogIC502\t\tF2\t\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("comp_2.draw"):
        os.remove("comp_2.draw")
    draw = open("comp_2.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 8 or (x_max - x_min) < 3:
            ...
            if x_min < -15 or x_max > 0:
                ...
                res.write(str(IC501_res)+"\t"+str(IC502_res)+"\t"+str(F2_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" symbol fill pattern "+str(d)+" \n")
                draw.write("s"+str(c)+" errorbar color "+str(d)+" \n")
                draw.write("s"+str(c)+" errorbar size 0.8 \n")
                draw.write("s"+str(c)+" line type 0 \n")
                draw.write("s"+str(c)+" legend \"log IC\\s501\\N="+str("{0:.2f}".format(IC501_calc))+' \\#{B1} '+str("{0:.2f}".format(IC501_err))+"; log IC\\s502\\N="+str("{0:.2f}".format(IC502_calc))+' \\#{B1} '+str("{0:.2f}".format(IC502_err))+"; F\\s2\\N="+str("{0:.2f}".format(F2_calc))+' \\#{B1} '+str("{0:.2f}".format(F2_err))+"; "+str(base)+"\" \n")
                #calculate fit
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save fit
                fit=(str(base)+'_comp_2_fit.data')
                ...
                #save curve
                curve=(str(base)+'_comp_2_curve.data')
