...
if count == 0:
    ...
    if os.path.isfile("competition_s.res"):
        os.remove("competition_s.res")
    res = open("competition_s.res","w")
    res.write("Competition screening\n\n")
    res.write("logIC50\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("comp_s.draw"):
        os.remove("comp_s.draw")
    draw = open("comp_s.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 3 or (x_max - x_min) < 2:
            ...
            if x_min < -15 or x_max > 0:
                ...
                IC50_estim = (x_min + x_max) * 0.5
                IC50_min = x_min - 1
                IC50_max = x_max + 1
                #fit data
                p0 = np.array([IC50_estim])
                def func(x, IC50):
                    return 100 - 100*10**x / (10**IC50 + 10**x)
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([IC50_min],[IC50_max]))
                perr = np.sqrt(np.diag(pcov))
                IC50_calc = (popt[0])
                IC50_err = (perr[0])
                #write results
                IC50_res = (str("{0:.2f}".format(IC50_calc))+' ± '+str("{0:.2f}".format(IC50_err)))
                res.write(str(IC50_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"log IC\\s50\\N="+str("{0:.2f}".format(IC50_calc))+' \\#{B1} '+str("{0:.2f}".format(IC50_err))+"; "+str(base)+"\" \n")
                #calculate curve
                y_fit = func(x_data, *popt)
                xy_fit = np.column_stack((x_data, y_fit))
                #save curve
                fit=(str(base)+'_comp_s_fit.data')
                ...
                #save curve
                curve=(str(base)+'_comp_s_curve.data')
    ...
    plt.title("Competition screening")
