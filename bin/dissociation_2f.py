...
if count == 0:
    ...
    if os.path.isfile("dissociation_2f.res"):
        os.remove("dissociation_2f.res")
    res = open("dissociation_2f.res","w")
    res.write("Dissociation from two sites.\nKoff1 fixed.\n\n")
    res.write("Koff1\tKoff2\t\t\tF2\t\t\tData file\n")
    #prepare file for Grace plot
    if os.path.isfile("dissoc_2f.draw"):
        os.remove("dissoc_2f.draw")
    draw = open("dissoc_2f.draw","w")
    c=-1 #plot counter
    d=0 #symbolcounter
    ...
    for file in sorted(glob.glob('*.dat')):
        ...
        if np.size(x_data) < 8:
            ...
            if x_min < 0 or y_max < 0 or y_max > 120:
                ...
                ask = ('Enter Koff1 fix value\nfor ' +str(file)+'\n')
                dlg = wx.TextEntryDialog(None, ask, "Input",'0.25')
                if dlg.ShowModal() == wx.ID_OK:
                    Koff1 = dlg.GetValue()
                dlg.Destroy()
                Koff2_estim = 0.693 / (x_max * 0.5)
                Koff2_min = 0.693 / (x_max * 2)
                Koff2_max = 0.693 / (x_max * 0.03)
                F2_estim = 50
                F2_min = 0
                F2_max = 100
                #fit data
                p0 = np.array([Koff2_estim, F2_estim])
                def func(x, Koff2, F2):
                    import numpy
                    global Koff1
                    Koff1 = float(Koff1)
                    return (100 - F2) * numpy.exp(-Koff1 * x) + F2 * numpy.exp(-Koff2 * x)
                popt, pcov = opt.curve_fit(func, x_data, y_data, p0, bounds=([Koff2_min, F2_min],[Koff2_max, F2_max]))
                ...
                res.write(str(Koff1)+"\t"+str(Koff2_res)+"\t"+str(F2_res)+"\t\t"+str(file)+"\n")
                #write Grace plot
                c=(c+1)
                d=(d+1)
                ...
                draw.write("s"+str(c)+" legend \"K\\sOff1\\N="+str(Koff1)+"; K\\sOff2\\N="+str("{0:.4f}".format(Koff2_calc))+' \\#{B1} '+str("{0:.4f}".format(Koff2_err))+"; F\\s2\\N="+str("{0:.0f}".format(F2_calc))+' \\#{B1} '+str("{0:.0f}".format(F2_err))+"; "+str(base)+"\" \n")
                #calculate fit
