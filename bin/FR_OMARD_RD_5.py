...
if columns == 0:
    ...
    set_4=os.path.basename(selected_4).split('/')[-1]
    res.write("\nLow receptor system\n")
    # load data
    x_data_4 = data_4[:,0]
    y_data_4 = data_4[:,1]
    ...
    if np.size(x_data_4) < 5 or (x_max_4 - x_min_4) < 2.5:
        ...
        if x_min_4 < -15 or x_max_4 > 0:
            ...
            if y_max_4 > y_max_max:
                y_max_max = y_max_4
            #calculate estimates
            tau4_estim = (y_max_4 - basal) / (Emax_estim - y_max_4)
            tau4_min = tau4_estim * 0.1
            tau4_max = tau4_estim * 10
            p4 = [tau4_estim, Emax_estim, logKA_estim, alpha_estim, beta_estim]
            #fit data
            p4_opt, p4_cov = opt.curve_fit(func, x_data_4, y_data_4, p4, bounds=([tau4_min, Emax_min, logKA_min, alpha_min, beta_min],[tau4_max, Emax_max, logKA_max, alpha_max, beta_max]))
            p4_err = np.sqrt(np.diag(p4_cov))
            tau4_calc = (p4_opt[0])
            Emax4_calc = (p4_opt[1])
            logKA4_calc = (p4_opt[2])
            alpha4_calc = (p4_opt[3])
            beta4_calc = (p4_opt[4])
            tau4_err = (p4_err[0])
            Emax4_err = (p4_err[1])
            logKA4_err = (p4_err[2])
            ...
            logKA4_res = (str("{0:.2f}".format(logKA4_calc))+' ± '+str("{0:.2f}".format(logKA4_err)))
            alpha4_res = (str("{0:.2f}".format(alpha4_calc))+' ± '+str("{0:.2f}".format(alpha4_err)))
            beta4_res = (str("{0:.2f}".format(beta4_calc))+' ± '+str("{0:.2f}".format(beta4_err)))
            res.write(str(tau4_res)+"\t"+str(Emax4_res)+"\t"+str(logKA4_res)+"\t"+str(set_4)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            draw.write("read xydy \""+str(file_1)+"\" \n")
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau4_calc))+' \\#{B1} '+str("{0:.4f}".format(tau4_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax4_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax4_err))+"; pK\\sA\\N="+str("{0:.2f}".format(logKA4_calc))+' \\#{B1} '+str("{0:.2f}".format(logKA4_err))+"; \\f{Symbol}a\\f{}="+str("{0:.4f}".format(alpha4_calc))+' \\#{B1} '+str("{0:.4f}".format(alpha4_err))+"; \\f{Symbol}b\\f{}="+str("{0:.4f}".format(beta4_calc))+' \\#{B1} '+str("{0:.4f}".format(beta4_err))+"; "+str(set_4)+"\" \n")
            #calculate curve
            fit=(str(base_4)+'_FR_OMARD_RD_fit.data')
            x_calc = np.linspace((x_min_4),(x_max_4), 121)
...
if columns == 0:
    ...
    set_5=os.path.basename(selected_5).split('/')[-1]
    res.write("\nLow receptor system\n")
    # load data
    x_data_5 = data_5[:,0]
    y_data_5 = data_5[:,1]
    ...
    if np.size(x_data_5) < 5 or (x_max_5 - x_min_5) < 2.5:
        ...
        if x_min_5 < -15 or x_max_5 > 0:
            ...
            if y_max_5 > y_max_max:
                y_max_max = y_max_5
            #calculate estimates
            tau5_estim = (y_max_5 - basal) / (Emax_estim - y_max_5)
            tau5_min = tau5_estim * 0.1
            tau5_max = tau5_estim * 10
            p5 = [tau5_estim, Emax_estim, logKA_estim, alpha_estim, beta_estim]
            #fit data
            p5_opt, p5_cov = opt.curve_fit(func, x_data_5, y_data_5, p5, bounds=([tau5_min, Emax_min, logKA_min, alpha_min, beta_min],[tau5_max, Emax_max, logKA_max, alpha_max, beta_max]))
            p5_err = np.sqrt(np.diag(p5_cov))
            tau5_calc = (p5_opt[0])
            Emax5_calc = (p5_opt[1])
            logKA5_calc = (p5_opt[2])
            alpha5_calc = (p5_opt[3])
            beta5_calc = (p5_opt[4])
            tau5_err = (p5_err[0])
            Emax5_err = (p5_err[1])
            logKA5_err = (p5_err[2])
            ...
            logKA5_res = (str("{0:.2f}".format(logKA5_calc))+' ± '+str("{0:.2f}".format(logKA5_err)))
            alpha5_res = (str("{0:.2f}".format(alpha5_calc))+' ± '+str("{0:.2f}".format(alpha5_err)))
            beta5_res = (str("{0:.2f}".format(beta5_calc))+' ± '+str("{0:.2f}".format(beta5_err)))
            res.write(str(tau5_res)+"\t"+str(Emax5_res)+"\t"+str(logKA5_res)+"\t"+str(set_5)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            draw.write("read xydy \""+str(file_1)+"\" \n")
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau5_calc))+' \\#{B1} '+str("{0:.4f}".format(tau5_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax5_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax5_err))+"; pK\\sA\\N="+str("{0:.2f}".format(logKA5_calc))+' \\#{B1} '+str("{0:.2f}".format(logKA5_err))+"; \\f{Symbol}a\\f{}="+str("{0:.4f}".format(alpha5_calc))+' \\#{B1} '+str("{0:.4f}".format(alpha5_err))+"; \\f{Symbol}b\\f{}="+str("{0:.4f}".format(beta5_calc))+' \\#{B1} '+str("{0:.4f}".format(beta5_err))+"; "+str(set_5)+"\" \n")
            #calculate curve
            fit=(str(base_5)+'_FR_OMARD_RD_fit.data')
            x_calc = np.linspace((x_min_5),(x_max_5), 121)
...
#Global fit
if np.any(x_data_1 != x_data_2) or np.any(x_data_1 != x_data_3) or np.any(x_data_1 != x_data_4) or np.any(x_data_1 != x_data_5):
    ...
    def err2_global(p, x, y1, y2, y3, y4, y5):
        p_21 = p[0], p[1], p[2], p[3], p[4]
        p_22 = p[5], p[1], p[2], p[3], p[4]
        p_23 = p[6], p[1], p[2], p[3], p[4]
        p_24 = p[7], p[1], p[2], p[3], p[4]
        p_25 = p[8], p[1], p[2], p[3], p[4]
        err_21 = err_2(p_21, x, y1)
        err_22 = err_2(p_22, x, y2)
        err_23 = err_2(p_23, x, y3)
        err_24 = err_2(p_24, x, y4)
        err_25 = err_2(p_25, x, y5)
        return np.concatenate((err_21, err_22, err_23, err_24, err_25))
    pg_estim = [p1_opt[0], Emax_estim, logKA_estim, alpha_estim, beta_estim, p2_opt[0], p3_opt[0], p4_opt[0], p5_opt[0]]
    pg_opt, cov_x, infodict, errmsg, success = opt.leastsq(err2_global, pg_estim, args=(x_data_1, y_data_1, y_data_2, y_data_3, y_data_4, y_data_5), full_output=1, epsfcn=0.0001)
    residuals = err2_global(pg_opt, x_data_1, y_data_1, y_data_2, y_data_3, y_data_4, y_data_5)
    ...
    pg4_opt = [pg_opt[7], pg_opt[1],pg_opt[2],pg_opt[3],pg_opt[4]]
    pg5_opt = [pg_opt[8], pg_opt[1],pg_opt[2],pg_opt[3],pg_opt[4]]
    tau1_calc = (pg_opt[0])
    tau1_err = (pg_err[0])
    Emax_calc = (pg_opt[1])
    Emax_err = (pg_err[1])
...
#write results
    ...
    tau4_res = (str("{0:.4f}".format(tau4_calc))+' ± '+str("{0:.4f}".format(tau4_err)))
    res.write(str(tau4_res)+"\t"+str(Emax_res)+"\t"+str(pKA_res)+"\t"+str(alpha_res)+"\t"+str(beta_res)+"\t"+str(set_4)+"\n")
    tau5_res = (str("{0:.4f}".format(tau5_calc))+' ± '+str("{0:.4f}".format(tau5_err)))
    res.write(str(tau5_res)+"\t"+str(Emax_res)+"\t"+str(pKA_res)+"\t"+str(alpha_res)+"\t"+str(beta_res)+"\t"+str(set_5)+"\n")
    ...
    #calculate curve 4
    fit=(str(base_4)+'_FR_OMARD_RD_gfit.data')
    x_calc = np.linspace((x_min_4),(x_max_4), 121)
    y_calc = func2_glob(x_calc, pg4_opt)
    xy_calc = np.column_stack((x_calc, y_calc))
    np.savetxt(fit,xy_calc,fmt='%.4e')
    # add to plot
    plt.plot(x_calc, y_calc, 'y', linestyle='--')
    ...
    draw.write("s"+str(c)+" line color 2 \n")
    draw.write("s"+str(c)+" layer 51 \n")
    draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau4_calc))+' \\#{B1} '+str("{0:.4f}".format(tau4_err))+str(set_4)+"\" \n")
    
    #calculate curve 5
    fit=(str(base_5)+'_FR_OMARD_RD_gfit.data')
    x_calc = np.linspace((x_min_5),(x_max_5), 121)
    y_calc = func2_glob(x_calc, pg5_opt)
    xy_calc = np.column_stack((x_calc, y_calc))
    np.savetxt(fit,xy_calc,fmt='%.4e')
    # add to plot
    plt.plot(x_calc, y_calc, 'y', linestyle='--')
    ...
    draw.write("s"+str(c)+" line color 2 \n")
    draw.write("s"+str(c)+" layer 51 \n")
    draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau5_calc))+' \\#{B1} '+str("{0:.4f}".format(tau5_err))+str(set_5)+"\" \n")
