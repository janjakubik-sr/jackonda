...
if columns == 0:
    ...
    if np.size(x_data_1) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            #calculate estimates
            Emax1_estim = (y_max - basal) * 2
            Emax1_min = basal + (Emax1_estim - basal) * 0.1
            Emax1_max = 1 + (Emax1_estim -1) * 10
            tau1_estim = 1
            tau1_min = 0.001
            tau1_max = 1000
...
#select file
dialog = wx.FileDialog(None, "Choose dat file of fourth partial agonist", os.getcwd(), "","*.dat", wx.FD_OPEN)
if dialog.ShowModal() == wx.ID_OK:
...
if columns == 0:
    ...
    if np.size(x_data_5) < 5 or (x_max - x_min) < 2.5:
        ...
        if x_min < -15 or x_max > 0:
            ...
            #Get agonist affinity
            ask = ('Enter pKA of agonist for '+str(file_5)+'\n')
            dlg = wx.TextEntryDialog(None, ask, "Input",'-6')
            if dlg.ShowModal() == wx.ID_OK:
                pKA5 = float(dlg.GetValue())
            dlg.Destroy()
            p5 = [tau5_estim, Emax5_estim]
            #fit data
            def func(x, tau5, Emax5):
                return basal + ((10**x) * tau5 * (Emax5 - basal)) / ((10**x) * (1 +tau5) + (10**pKA5))
            ...
            res.write(str(tau5_res)+"\t"+str(Emax5_res)+"\t"+str(pKA5)+"\t"+str(file_5)+"\n")
            #write Grace plot
            c=(c+1)
            d=(d+1)
            ...
            draw.write("s"+str(c)+" errorbar size 0.8 \n")
            draw.write("s"+str(c)+" line type 0 \n")
            draw.write("s"+str(c)+" legend \"\\f{Symbol}t\\f{}="+str("{0:.4f}".format(tau5_calc))+' \\#{B1} '+str("{0:.4f}".format(tau5_err))+"; E\\sMAX\\N="+str("{0:.2f}".format(Emax5_calc))+' \\#{B1} '+str("{0:.2f}".format(Emax5_err))+"; pK\\sA\\N="+str("{0:.2f}".format(pKA5))+"; "+str(file_5)+"\" \n")
            #calculate curve
            x_calc = np.arange(x_min, x_max, step)
...
if np.any(x_data_1 != x_data_2):
    ...
    pg5_opt = [pg_opt[5], pg_opt[1], pKA5]
    tau1_calc = (pg_opt[0])
    tau1_err = (pg_err[0])
    Emax1_calc = (pg_opt[1])
    Emax1_err = (pg_err[1])
